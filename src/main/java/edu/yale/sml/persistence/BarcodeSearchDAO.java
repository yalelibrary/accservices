    ...
    public List<SearchResult> findAllById(List<String> id)
    {
        ...
        try
        {
            try
            {
                for (String s : id)
                {
                    SQLQuery q = session.createSQLQuery("select b.ITEM_BARCODE, s.ITEM_ID, s.ITEM_STATUS_DATE, st.ITEM_STATUS_DESC, mi.YEAR, mi.CHRON," +
                    		" mi.ITEM_ENUM, mi.MFHD_ID, mM.NORMALIZED_CALL_NO," + " mM.DISPLAY_CALL_NO, mM.SUPPRESS_IN_OPAC, mM.ENCODING_LEVEL, " +
                    				"i.PERM_LOCATION, l.LOCATION_NAME, mM.CALL_NO_TYPE "
                            + " from YALEDB.ITEM i inner join YALEDB.ITEM_BARCODE b " + "on i.ITEM_ID = b.ITEM_ID" + " left outer join YALEDB.ITEM_STATUS s on i.ITEM_ID=s.ITEM_ID " + " left outer join YALEDB.ITEM_STATUS_TYPE st on s.ITEM_STATUS = st.ITEM_STATUS_TYPE  "
                            + " left outer join YALEDB.MFHD_ITEM mI on i.ITEM_ID = mi.ITEM_ID" + " left outer join YALEDB.MFHD_MASTER mM on mi.MFHD_ID = mM.MFHD_ID" + " left outer join YALEDB.LOCATION l on i.PERM_LOCATION=l.LOCATION_ID" + " where b.ITEM_BARCODE = :param");
